# 포지트론 IDE {#sec-positron}

[4장](#sec-data-science-ide)에서 살펴본 것처럼, 데이터 과학 IDE는 분리된 생태계에서 통합 환경으로, 그리고 AI 네이티브 시대로 진화해왔다. 이러한 진화의 정점에 있는 것이 바로 **포지트론\index{포지트론}(Positron)**이다. 본 장에서는 포지트론을 실제로 설치하고 설정하여 AI 어시스턴트, 단축키, 문서 제작 도구 등을 활용하는 구체적인 방법을 다룬다.

**포지트론(Positron)**은 Posit(구 RStudio\index{RStudio})에서 개발한 차세대 데이터 과학 통합개발환경(IDE)으로, 2024년 정식 출시되었다. 마이크로소프트의 오픈소스 프로젝트인 [Code OSS](https://github.com/microsoft/vscode)(VS Code\index{VS Code}의 기반)를 토대로 개발되었으며, 기존 RStudio IDE의 핵심 기능들을 현대적으로 재구현했다.

## 포지트론 등장 배경 {#sec-positron-background}

데이터 과학 분야에서 IDE\index{IDE}는 크게 RStudio와 Jupyter\index{Jupyter} 진영으로 나뉘어 발전해왔다. 각 IDE는 고유한 장점을 가지고 있었지만, Visual Studio Code의 등장과 AI 기능의 통합으로 데이터 과학 개발 환경에 큰 변화가 일어났다. 이러한 변화에 대응하여 Posit은 다음과 같은 목표로 포지트론을 개발했다:
...
![데이터 사이언스 IDE 생태계](images/ide_war.jpg){#fig-ide-war fig-align="center" width="250"}
...
### 제한사항과 향후 전망 {#sec-limitations-future}
